...
class Unet_model(nn.Module):
    ...
    def forward(self, inx):
        ...
        infm_res=self.mask_encoder(inx)["512"]
        infm=infm+infm_res
        mask=self.mask_decoder(infm)
        mask=torch.nn.functional.interpolate(mask,size=(img_size[-2],img_size[-1]))
      
        return mask
      
##write a test case of unet_model
# import torch
model=Unet_model()
model.eval()
