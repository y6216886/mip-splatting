...
class LinearLayer(torch.nn.Module):
    ...
    def pos_enc(self, features):
        return positional_encoding(features, self.feape)
    def forward(self, features, sum_w):
        ...
        # features = features * sum_w
        features=torch.einsum("ij,ik->ij", features, sum_w)

        indata = [features]
        # if self.feape > 0:
        #     indata += [positional_encoding(features, self.feape)]
        mlp_in = torch.cat(indata, dim=-1)
        out = self.layer(mlp_in)
        # out=self.bn(out)
        # out = mlp_in @ self.layer.weight.T #+ sum_w * self.layer.bias[None,...] # [H*W, out_dim]
        # out = mlp_in @ self.layer.weight.T + self.layer.bias[None,...]
